...
const Dashboard = () => {
  ...
  const { darkMode, expenses } = useAppSelector((state) => state.ui);
  ...
  return (
    <>
      <div className={`flex lg:flex-row sm:flex-col md:flex-row `}>
        <div className="flex flex-col gap-y-12 lg:ml-48 lg:mt-24 sm:mt-20">
          <Cards />
          <div
          ...
          <div>
            <ul className="flex lg:flex-row sm:flex-col lg:gap-x-8 sm:gap-y-4 sm:ml-4 -mt-8">
              {topExpenses.map((expense, index) => (
                <li
                  key={index}
                  className={`  w-32 px-2 h-16 border border-blue-700 pl-2 overflow-x-hidden ${
        ...
        <div className="flex flex-col gap-y-4 ml-12 mt-32">
          <div
            className={` w-64 h-96 rounded-md pl-2 pr-1 ${
              darkMode ? "bg-gray-300" : "bg-gray-100"
            }`}
          >
            <h2 className="font-semibold text-green-700">Revenue sources</h2>
            {revenues.some((revenue : any) => revenue.amount > 0) ? (
              <ul className=" mt-4 gap-y-4 pb-1">
                {" "}
